...
export const GameTable: React.FC = () => {
  ...
  return (
    <div className="h-screen bg-[#0A0A0B] text-slate-200 flex overflow-hidden font-sans">
      {/* Left Sidebar: Stats & Activity */}
      <aside className="w-72 bg-[#121214] border-r border-slate-800 flex flex-col p-6 z-30">
        ...
        <div className="space-y-6 flex-1 overflow-hidden flex flex-col">
          {/* Scoreboard */}
          <div className="bg-[#1A1A1D] rounded-2xl p-5 border border-slate-800 transition-all hover:border-slate-700 shadow-xl">
            ...
            <div className="mt-5 pt-4 border-t border-slate-800 flex justify-between text-[10px] font-bold uppercase tracking-wider">
              ...
              <span className="text-emerald-400">T1: {game.tricksWon[0]} | T2: {game.tricksWon[1]}</span>
            </div>
          </div>
          ...
          <div className="space-y-3 px-1">
            <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
              ...
              <span className="text-slate-100 font-mono">#{game.roomId.toUpperCase()}</span>
            </div>
            <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
              <span className="text-slate-500">Phase</span>
              <span className="text-emerald-500">{game.phase.replace('_', ' ')}</span>
            </div>
          </div>

          {/* Activity Logs */}
          <div className="flex-1 bg-[#0A0A0B] rounded-2xl border border-slate-800 p-4 overflow-hidden flex flex-col shadow-inner">
            <p className="text-[10px] uppercase text-slate-600 font-black tracking-widest mb-4">Tactical Feed</p>
            <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1">
              {game.history.slice(-10).map((log, i) => (
                <div key={i} className="text-[10px] leading-relaxed text-slate-400 border-l border-emerald-500/30 pl-3">
                  {log}
                </div>
              ))}
            </div>
          ...
          {game.spectators.length > 0 && (
            <div className="mt-4 p-4 bg-black/20 rounded-2xl border border-slate-800/50">
        ...
        <div className="mt-8 pt-6 border-t border-slate-800">
           <button onClick={() => window.location.reload()} className="w-full py-3 bg-slate-800 hover:bg-slate-700 rounded-xl text-[10px] font-black tracking-widest transition-all active:scale-95">LEAVE TABLE</button>
        </div>
      </aside>

      {/* Main Game Stage */}
      <main className="flex-1 relative bg-[#121214] flex flex-col p-4">
        ...
        <div className="absolute top-8 left-1/2 -translate-x-1/2 z-20">
          <div className="bg-black/40 backdrop-blur-xl px-6 py-2 rounded-full border border-slate-700/50 flex items-center gap-4 shadow-2xl">
            <span className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Trump Active</span>
        ...
        {/* The Table Stage */}
        <div className="flex-1 flex items-center justify-center px-12 py-20 pointer-events-none">
          <div className="relative w-full max-w-4xl aspect-[16/10] bg-[#0E3524] rounded-[240px] border-[16px] border-[#1C1C1E] shadow-[0_0_120px_rgba(0,0,0,0.8),inset_0_0_80px_rgba(0,0,0,0.5)] flex items-center justify-center">
             {/* Center Play Area */}
             <div className="relative w-[40%] aspect-square rounded-full flex items-center justify-center border border-white/5 bg-black/5">
                <AnimatePresence>
                    {game.currentTrick.map((play) => {
                        const relIdx = getRelativePos(game.players.find(p => p.id === play.playerId)!.pos);
                        const offsets = [
                            { y: 80, x: 0, r: 0 },   // Bottom
                            { y: 0, x: -80, r: 0 }, // Left (Horizontal design says 0 deg but we can tilt slightly)
                            { y: -80, x: 0, r: 0 },  // Top
                            { y: 0, x: 80, r: 0 },   // Right
                        ];
                        ...
                        return (
                            ...
                            >
                                <Card card={play.card} disabled className="shadow-2xl scale-[1] border-2 border-slate-100" />
                            </motion.div>
                        );
                    })}
                </AnimatePresence>
                {game.currentTrick.length === 0 && game.phase === 'PLAYING' && (
                    <div className="text-[10px] font-black uppercase text-white/10 tracking-[0.3em] text-center max-w-[120px]">
                        Waiting for play
                    </div>
                )}
             </div>
             ...
             {playersByPos.map((p: Player | null, i) => {
                ...
                return (
                    <div key={p.id} className={`absolute ${posStyles[i]} z-20 flex flex-col items-center gap-2`}>
                        <div className={cn(
                            "w-16 h-16 rounded-full border-2 p-1 bg-[#121214] overflow-hidden transition-all duration-300",
                            isCurrent ? "scale-110 shadow-[0_0_20px_rgba(16,185,129,0.5)] border-emerald-500" : `${teamColor} opacity-80 shadow-2xl`
                        )}>
                            <div className="w-full h-full rounded-full bg-slate-800 flex items-center justify-center font-black text-xl text-slate-100">
                                {p.name[0].toUpperCase()}
                            </div>
                        </div>
                        <div className={cn(
                            "px-4 py-1.5 rounded-full border border-slate-700/50 backdrop-blur-md flex flex-col items-center",
                            isCurrent ? "bg-emerald-500/10 border-emerald-500/30" : "bg-black/40"
                        )}>
                            ...
                            <div className="flex gap-1 mt-1.5">
                                {Array(p.handSize).fill(0).map((_, idx) => (
                                    <div key={idx} className="w-1.5 h-1.5 bg-white/30 rounded-full" />
                                ))}
                            </div>
                        </div>
                    </div>
        ...
        <div className={cn(
            "h-64 bg-gradient-to-t from-black/60 to-transparent flex flex-col items-center justify-end pb-12 z-30",
            isSpectator ? "pointer-events-none opacity-50 grayscale pt-10" : "pointer-events-none"
        )}>
          {isSpectator ? (
             <div className="mb-10 text-center">
                <div className="inline-flex items-center gap-3 px-6 py-3 bg-slate-900 border border-slate-800 rounded-full shadow-2xl">
                    <Eye size={16} className="text-emerald-500" />
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Spectator Feed Active</span>
                </div>
             </div>
          ) : (
            <div className="flex -space-x-8 sm:-space-x-4 pointer-events-auto">
                <AnimatePresence>
                    {me.hand.map((card, idx) => (
                        <motion.div
                            key={card.id}
                            initial={{ y: 150, rotate: (idx - (me.hand.length / 2)) * 5 }}
                            animate={{ y: 0, rotate: (idx - (me.hand.length / 2)) * 2 }}
                            whileHover={{ y: -30, rotate: 0, zIndex: 50 }}
                        ...
                        >
                            <Card 
                                ...
                                disabled={!isMyTurn}
                                className={cn(
                                    "ring-offset-2 ring-offset-[#121214] transition-all",
            ...
            <AnimatePresence>
                {isMyTurn && (
                    <motion.div 
                        ...
                        className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/50 px-6 py-2 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.2)]"
                    >
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white">Your Strategem</span>
                    </motion.div>
                )}
            </AnimatePresence>
            <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-2">
                <Clock size={12} className="text-slate-700" />
                Network: <span className="text-emerald-500 font-mono">STABLE</span>
            </div>
          </div>
        </div>
